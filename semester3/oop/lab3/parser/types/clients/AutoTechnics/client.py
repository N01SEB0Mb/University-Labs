...
class AutoTechnics(BaseClient):
    """
    AutoTechnics parser client, inherits BaseClient
    ...
    def search(
        ...
        try:
            ...
            assert getSearchResults.status_code == 200
        ...
        items = getSearchResults.json()["items"]
        ...
        # Verifying "found by" value

        for itemNumber, itemInfo in enumerate(items):
            if self.onlyMatchingArt:
                if not self.compareArticles(
                        article,
                        itemInfo["Item"]
                ):
                    foundBy = []

                    for by in getSearchResults.json()["by"]:
                        if by["item"] == itemInfo["Item"]:
                            foundBy.append(by["brand"] + ": " + by["search"])

                    if foundBy:
                        itemInfo["Название"] += " (Найдено по: {})".format(", ".join(foundBy))
                    else:
                        continue

            # Adding item if it was found for specified article

            response.append(
                ResponseSearch.Item(
                    article=itemInfo["Item"],
                    brand=itemInfo["Бренд"],
                    name=itemInfo["Название"],
                    description=itemInfo["Описание"],
                    price="%.2f UAH" % itemInfo["Retail"]
                )
            )
    ...
    def info(
        ...
        try:
            ...
            for foundAnalogItem in foundAnalogsInfo:
                ...
                try:
                    for image in foundAnalogItem["images"]:
        ...
        analogsId = {responseItem.id}

        for analogItem in responseItem["analogs"]:
            analogsId.add(analogItem.id)

        return self.__searchByAnalogs(resultDict, article, brand, responseItem, analogsId)
